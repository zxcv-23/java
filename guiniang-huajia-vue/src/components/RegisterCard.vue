...
const emit = defineEmits(['celebrate', 'switch'])
...
function onSubmit() {
  ...
  loading.value = true
  try {
    // 调用后端注册接口（Spring Boot + MyBatis-Plus）
    const res = await fetch('http://127.0.0.1:8081/api/user/register', {
      method: 'POST',
    ...
    if (res.ok && json.code === 200) {
      ...
      loading.value = false
      showHint(json.message || '录入失败，请稍后再试')
    }
  } catch (e) {
    loading.value = false
    showHint('冥府信道受阻，请确认后端已启动')
  }
}
</script>
